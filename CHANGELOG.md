...
### Changed
- **Containerized Admin Console integration**: Named the admin tool after the application (`snake`) and copied it to the container's system path `/usr/local/bin/snake`. The TUI can now be launched by simply executing `snake tui` (or `snake`) inside the container shell.
...
- **Rebrand to studio2201**: README, container labels, docker-compose, and Cargo
  metadata now reference `studio2201/snake`. CI badge URL and GHCR image name
  updated accordingly.
- **Fixed truncated comment block** in `docker-compose.yml` (the closing
  parenthesis and "Configure the deployment…" sentence were missing).
- **Favicon cache-bust query** bumped `?v=1.0.37` → `?v=1.0.54` in
